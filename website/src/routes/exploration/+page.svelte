...
<div class="content">
	...
	<section class="exploration-section" id="data-collection">
		...
		<p>During the data collection step, we opted not to use a dynamic API to collect our data. We opted not to do this for many reasons. For starters, there are latency challenges, security challenges, as well as overall availability issues. Getting a business to agree, or anyone in fact, to give us real-time access to live streamed transactions is a major security risk and would cost any company money. During this we also explored the idea of using real-time crypto transactions as our data source but decided not to pursue this route. That idea came up because it would give us a real-time transactions feed. It does not, however, provide a fraud label. Without labels, we would not be able to evaluate supervised models or fairly compare these models with unsupervised models.</p>
		<p>Even though we could not get a live API, the data set we ultimately chose consisted of fraud data in bank account creation applications, which was available through a public API. The data set chosen was the Bank Account Fraud (BAF) dataset by Feedzai. Feedzai is a real-world fraud detection tool that works with various institutions. This means that the data being utilized is not simply a random CSV file found on the internet. This data set was first introduced in a NeurIPS 2022 paper and claims to be built using privacy preserving methods that were applied to real bank account application fraud data. The fact that this data is altered does not mean it is unusable. It was modeled after real fraud behavior patterns and designed to be used specifically for training machine-learning models in a financial setting.</p>
		<p>This dataset fits our research question perfectly because our scope is about comparing supervised fraud detection models to unsupervised anomaly detection under extreme class imbalance. The BAF dataset has transaction-level features and a clear fraud label (fraud_bool), which lets us train supervised models and test anomaly detection methods side by side. The data set, as expected, has large class imbalance with only about 1% of the data being classified as fraud. This very closely mimics a real world setting. This imbalance is quite literally the core challenge we are studying in this project. The data set supports the goals of our project without us having to manipulate the data artificially.</p>
	</section>
	...
	<section class="exploration-section" id="data-understanding">
		...
		<p>Each row in the data set represents a single account application. Each account application was submitted by a user who was attempting to open a financial account or line of credit. This data set has numerical features, categorical features and binary features. The numerical features in this data set include income, customer_age, credit_risk_score, session_length_in_minutes and proposed_credit_limit. These are all great numerical variables that can be used in analysis and various machine learning models. Some of the categorical features include payment_type, employment_status, housing_status and device_os. These variables will require encoding methods such as one hot encoding before they can be used in machine learning algorithms. Finally, we have binary features that can only be 0 or 1. Some examples from that data include: fraud_bool, email_is_free, phone_home_valid, foreign_request and has_other_cards. Since binary variables are not categorical, we will not need to require additional encoding on these values.</p>
	</section>
	...
	<section class="exploration-section" id="cleaning">
		...
		<p>The goal for this milestone was to get the dataset (Feedzai) to a state where it can be used prior to modeling, which will take place in Milestone 3. We came across a mix of numerical and categorical fields in the raw file as well as columns which contain a placeholder for missing values (–1). We converted those to proper missing values so that they could be handled correctly.</p>
		<p>Numeric columns were filled using the median since several features are skewed and the median is less sensitive to extreme values. Categorical columns were filled with the most common category in each column. After that, numeric features were standardized and normalized so that they would be on a similar scale. Categorical features were one hot encoded so they can be used later in machine-learning models.</p>
		...
		<p>For the , we have not done any dimensionality reduction. We are proceeding with all available features and option for a "backward selection" method of model refinement. We believe keeping all the features intact will make it easier to understand which variables are driving our fraud predictions. Our data also does not have an excessive number of features. With this in mind, we assessed that removing features early might cause us to lose important signals. During Milestone Three, in model creation, we anticipate applying dimensionality reduction both to remove noise and to optimize our model.</p>
	...
	<section class="exploration-section" id="correlations">
		...
		<ul>
			...
			<li>Our key feature though, fraud_bool, shows no obvious "smoking gun." The highest correlations that are visible are those that have already been addressed in the previous section: customer_age, credit_risk_score, income, has_other_cards, name_email_similarity, and date_of_birth_distinct_emails_4w.</li>
		</ul>
	</section>
		...
		<div class="table-scroll">
			<table>
				...
				<tbody>
					...
					<tr><td>Count of Months with Current Bank</td><td>253,635</td><td>25.36%</td><td>Large gap: Over 25% of data missing for this feature. Is this indicative of new customers?</td></tr>
