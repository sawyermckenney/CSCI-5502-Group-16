	...
	const cards: CardData[] = [
		...
		},
		{
			...
			content: [
				...
				text(`From Model 2, additional insights were attained beyond pure predictions. The Decision Tree model made it clear that the detection of fraud was greatly influenced by features such as transaction velocity, session and financial features like credit risk score and credit limit. This model was capable of achieving a high recall that approximated at 74%. The implication here is that this model was capable of capturing a large portion of fraud cases, but precision, unfortunately remained low (~3%), resulting in a low F1-score (~0.06). This clearly shows the impact of class imbalance. Here, the model prioritizes catching the fraud at the cost of a higher number of false positives. Pitting Model 2’s results against those of Model 3 showed a more balanced improvement after tuning. This was particularly evident n terms of F1-score and precision. Model 2 prioritized recalls (~74%) but suffered from extremely low precision (~3%), leading to a much lower F1-score (~.065), whereas Model 3 increased the recall to around 26% while improving precision, resulting in a higher F1-score (~.14). This contrast shows the two different modeling trade-offs. Model 3’s approach aimed for a more balanced classifier whilst Model 2 focused more heavily on capturing as much fraud as possible, even if false positive rates were high. Despite the difference, both models show similar important features such as housing status and device type, reinforcing that fraud is driven by a mix of behavioral and identity-related factors.`),
			]
		}
	];
