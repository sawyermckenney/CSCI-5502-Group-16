...
<div class="content">
	...
	<section class="exploration-section" id="data-collection">
		...
		<p>Even though we could not get a live API, the data set we ultimately chose consisted of fraud data in bank account creation applications, which was available through a public API. The data set chosen was the Bank Account Fraud (BAF) dataset by Feedzai. Feedzai is a real-world fraud detection tool that works with various institutions. This means that the data being utilized is not simply a random CSV file found on the internet. This data set was first introduced in a NeurlIPS 2022 paper and claims to be built using privacy preserving methods that were applied to real bank account application fraud data. Since this data is altered, it does not mean it is unusable. It was modeled after real fraud behavior patterns and designed to be used specifically for training machine-learning models in a financial setting.</p>
